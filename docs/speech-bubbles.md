...
640 px (`tools/training/build.py`, `label.py`; the whole pipeline is in
`docs/training.md`). The current student was
...
(the corpus is private and not distributed; only the annotations under
`tools/training/gt/` ship with the repository):
...
captions, which added 57 boxes on 33 pages
(`tools/training/gt/v4_bubble_corrections.json`:
...
was invisible to both). `tools/training/gt/silhouettes.json` holds 115
...
The polygons were produced by `tools/training/silhouettes.py` at full
...
With a `boxes.json` in the folder (the `tools/training/detect_boxes.py` dump format:
